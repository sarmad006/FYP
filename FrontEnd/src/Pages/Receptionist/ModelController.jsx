...
const ModelController = () => {
  ...
  async function getGlobalModels() {
    ...
    const Contract = new ethers.Contract(modelAddress, abi, signer);
              ...
              {models.map((item) => (
                <div className="flex flex-row text-white justify-between  mt-4 py-4 px-16 rounded-xl Div2 font-poppins">
                  ...
                  <div className="flex gap-x-12">
                    <button
                      className="bg-limgreen px-8 rounded-full text-black font-medium"
                      onClick={() => handleClick2(item)}
                    >
                      Download
                    </button>
                    <button
                      className="bg-slate-100 px-8 rounded-full text-black font-medium"
